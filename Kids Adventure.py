...
pygame.display.set_caption('Kids Adventure')
...
class Player():
    ...
    def update(self, game_over):
        ...
        if game_over == 0:
            ...
            #add gravity
            self.vel_y += 1     #positive velocity means player is jumping, negative velocity means player is falling 
            if self.vel_y > 10: #due to gravity
                self.vel_y = 10
            dy += self.vel_y 
            ...
            for tile in world.tile_list:
                #check for collision in x direction / X axis
                if tile[1].colliderect(self.rect.x + dx , self.rect.y, self.width, self.height):
                    dx = 0
                #check for collision in y direction / Y axis
    ...
    def reset(self, x, y):      #re-initialize the player positon / reset
        self.images_right = []
        self.images_left = []
        self.index = 0
        self.counter = 0
        for num in range(1,5):
            img_right = pygame.image.load(f'Assets/guy{num}.png') #load all player images
